...
check_agent <- function(irf, rf, match_agents = FALSE){
  ...
  assertthat::assert_that(length(unique(rf[['agent']])) == 1, msg = 'rf can only contain information for one agent')
  assertthat::assert_that(length(unique(irf[['agent']])) == 1, msg = 'irf can only contain information for one agent')

  if(match_agents){

    assertthat::assert_that(unique(rf[['agent']]) == unique(irf[['agent']]), msg = 'irf and rf agent must be the same if match_agents = TRUE')
...
check_time <- function(irf, rf){
  ...
  assertthat::assert_that(abs(diff_by) == 0, msg = paste0('irf and rf differ by ', diff_by, ' time steps.'))

  assertthat::assert_that(any(!duplicated(irf[['year']])), msg = 'irf cannot contain multiple entries for the same year')
  assertthat::assert_that(any(!duplicated(rf[['year']])), msg = 'rf cannot contain multiple entries for the same year')

  irf_step <- diff(irf[['year']])
  rf_step  <- diff(rf[['year']])

  assertthat::assert_that(length(unique(irf_step)) == 1, msg = 'irf must contain a constant time step size')
  assertthat::assert_that(length(unique(rf_step)) == 1, msg = 'rf must contain a constant time step size')

  assertthat::assert_that(all(identical(irf_step, rf_step)), msg = 'irf and rf must contain the same time step size')
...
check_config_matrix <- function(m){
  ...
  assertthat::assert_that(is.matrix(m), msg = 'config_matrix must be a matrix')
  assertthat::assert_that(all(m %in% c(0, 1)), msg = 'config_matrix can only contain entries of 0 and 1')
  assertthat::assert_that(any(1 %in% m), msg = 'config_matrix is missing a 1 entry, HIRM will not run')
  assertthat::assert_that(!is.null(colnames(m)), msg = 'config_matrix is missing column names, should be the irf names')
  assertthat::assert_that(!is.null(row.names(m)), msg = 'config_matrix is missing row names, should be the rf time series names')
